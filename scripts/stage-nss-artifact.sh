...
[[ "$NEXAWRT_FLAVOR" == nss ]] || { echo "Refusing NSS artifact staging for flavor: $NEXAWRT_FLAVOR" >&2; exit 1; }

WORK_DIR="${WORK_DIR:-$ROOT_DIR/.work/openwrt-nss}"
BUILD_LOG="${BUILD_LOG:-$ROOT_DIR/build-nss.log}"
DIST_DIR_RAW="${DIST_NSS_DIR_OVERRIDE:-$ROOT_DIR/dist-nss}"
...
actual_source_commit="$(git -C "$WORK_DIR" rev-parse --verify HEAD 2>/dev/null)" || fail "NSS source checkout has no valid HEAD"
actual_source_origin="$(git -C "$WORK_DIR" remote get-url origin 2>/dev/null)" || fail "NSS source checkout has no origin"
...
[[ "$(git -C "$WORK_DIR" rev-parse --verify HEAD 2>/dev/null)" == "$actual_source_commit" ]] ||
  fail "NSS source HEAD changed during validation"
[[ "$(git -C "$WORK_DIR" remote get-url origin 2>/dev/null)" == "$actual_source_origin" ]] ||
  fail "NSS source origin changed during validation"
...
"$ROOT_DIR/scripts/collect-build-evidence.sh" nss "$WORK_DIR" "$DIST_DIR" "$BUILD_LOG"
...
[[ "$(find "$DIST_DIR/LICENSES" -type f -print)" == "$DIST_DIR/LICENSES/nss-firmware/LICENSE.md" ]] || fail "license directory contains a non-allowlisted file"
...
printf 'NexaWrt NSS RAM-boot-only artifact files written to %s\n' "$DIST_DIR"
