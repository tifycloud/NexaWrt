...
CATALOG_STAGER="$ROOT_DIR/scripts/validate-component-catalogs.py"
CATALOG_STAGING_TEST="$ROOT_DIR/tests/test_component_catalog_staging.py"
SITE="$ROOT_DIR/site"
CATALOG="$ROOT_DIR/components/catalog.json"

for path in "$WORKFLOW" "$GENERATOR" "$PROOF_VERIFIER" "$PROOF_TEST" "$DEVICE_VALIDATOR" "$DEVICE_METADATA" "$DEVICE_TEST" "$UI_TEST" "$CATALOG_STAGER" "$CATALOG_STAGING_TEST" "$CATALOG" "$SITE/index.html" "$SITE/styles.css" "$SITE/app.js" "$SITE/favicon.svg" "$SITE/releases.json" "$SITE/.nojekyll"; do
...
test -x "$DEVICE_TEST"
test -x "$CATALOG_STAGER"
test -x "$CATALOG_STAGING_TEST"
python3 -m py_compile "$GENERATOR" "$PROOF_VERIFIER" "$PROOF_TEST" "$DEVICE_VALIDATOR" "$DEVICE_TEST" "$CATALOG_STAGER" "$CATALOG_STAGING_TEST"
python3 "$PROOF_TEST"
python3 "$DEVICE_TEST"
...
grep -Fq -- '--components-root components' "$WORKFLOW"
grep -Fq -- '--site-root site/components' "$WORKFLOW"
grep -Fq 'python3 tests/test_component_catalog_staging.py' "$WORKFLOW"
grep -Fq 'MAX_SHARDS = 3' "$CATALOG_STAGER"
grep -Fq 'MAX_CURATED_CATALOG_BYTES = 2 * 1024 * 1024' "$CATALOG_STAGER"
grep -Fq 'MAX_PACKAGE_INDEX_BYTES = 2 * 1024 * 1024' "$CATALOG_STAGER"
grep -Fq 'MAX_PACKAGE_SHARD_BYTES = 16 * 1024 * 1024' "$CATALOG_STAGER"
grep -Fq 'object_pairs_hook=duplicate_hook' "$CATALOG_STAGER"
grep -Fq 'resolver.validate_catalog' "$CATALOG_STAGER"
grep -Fq 'resolver.validate_package_catalog_index' "$CATALOG_STAGER"
grep -Fq 'resolver.validate_package_shard' "$CATALOG_STAGER"
grep -Fq 'len(expected_shards) != MAX_SHARDS' "$CATALOG_STAGER"
grep -Fq 'hashlib.sha256(shard_raw).hexdigest()' "$CATALOG_STAGER"
grep -Fq 'stat.S_ISLNK' "$CATALOG_STAGER"
grep -Fq 'stat.S_ISREG' "$CATALOG_STAGER"
grep -Fq 'st_nlink != 1' "$CATALOG_STAGER"
grep -Fq 'os.O_NOFOLLOW' "$CATALOG_STAGER"
grep -Fq '_write_validated_file(staged_packages / relative_name, staged_packages, shard_raw)' "$CATALOG_STAGER"
grep -Fq 'del shard_payload, shard, shard_raw' "$CATALOG_STAGER"
grep -Fq 'test_rejects_extra_root_key' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_duplicate_json_key' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_package_index_catalog_version_mismatch' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_shard_target_mismatch' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_shard_flavor_mismatch' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_shard_catalog_version_mismatch' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_package_id_mismatch' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_undeclared_feed' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_forged_risk' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_base_files_reclassified_as_selectable' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_non_noarch_record_in_nss' "$CATALOG_STAGING_TEST"
grep -Fq 'test_rejects_hardlinked_source_shard' "$CATALOG_STAGING_TEST"
grep -Fq "find site/components \( -type l -o -type f -links +1 \)" "$WORKFLOW"
if grep -Eq 'cp[[:space:]]+-a|cp[[:space:]]+(-[^[:space:]]*[LR][^[:space:]]*)?[[:space:]]+components/packages' "$WORKFLOW"; then
  echo 'Pages workflow must not blindly copy or follow package catalog links' >&2
  exit 1
fi
